    ...
    switch ($action) {
        ...
        case 'update_location':
            ...
            if (!empty($pin_location)) {
                ...
                update_post_meta($order_id, '_billing_location', $pin_address);
        ...
        case 'save_order_details':
            ...
            // Also update billing fields for compatibility
            update_post_meta($order_id, '_billing_deceased_name', $deceased_name);
            update_post_meta($order_id, '_billing_deceased_date_of_death', $date_of_death);
            update_post_meta($order_id, '_billing_symmetry_name', $cemetery);
            update_post_meta($order_id, '_billing_how_heard_about_us', $how_hear);
            update_post_meta($order_id, '_billing_location', $pin_address);
...
// Get product names for cleaning type
 $product_names = array();
foreach ($order_items as $item_id => $item) {
 ...
 $cleaning_type_value = implode(', ', $product_names);
...
// Get custom fields from billing fields (these are the extra fields added to checkout)
 $billing_symmetry_name = get_post_meta($order_id, 'billing_symmetry_name', true);
 $billing_location = get_post_meta($order_id, 'billing_location', true);
 $billing_how_heard_about_us = get_post_meta($order_id, 'billing_how_heard_about_us', true);
 $billing_deceased_name = get_post_meta($order_id, 'billing_deceased_name', true);
 $billing_deceased_date_of_death = get_post_meta($order_id, 'billing_deceased_date_of_death', true);

// Also get from regular meta fields as fallback
...
<body class="bg-gray-50">
  ...
  <header class="bg-white border-b border-gray-200 sticky top-0 z-40">
    <div class="max-w-7xl mx-auto px-6 py-5">
      <div class="flex items-center justify-between bh-detail-page-header">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
            ...
            <p class="text-sm font-medium text-gray-900">
                <?php 
                $location = get_post_meta($order_id, '_billing_location', true);
                if (empty($location)) {
                    $location = get_post_meta($order_id, 'pin_address', true);
                }
                echo esc_html($location ? $location : 'Not set');
                ?>
            </p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
            ...
            <input type="text" 
                   id="deceased_name" 
                   name="deceased_name" 
                   value="<?php echo esc_attr(get_post_meta($order_id, '_billing_deceased_name', true)); ?>" 
                   class="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-900">
          </div>
          ...
          <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
            ...
            <input type="date" 
                   id="date_of_death" 
                   name="date_of_death" 
                   value="<?php echo esc_attr(get_post_meta($order_id, '_billing_deceased_date_of_death', true)); ?>" 
                   class="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-900">
          </div>

          <!-- Cemetery (Symmetry Name) -->
          <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Cemetery</p>
            <input type="text" 
                   id="cemetery" 
                   name="cemetery" 
                   value="<?php echo esc_attr(get_post_meta($order_id, '_billing_symmetry_name', true)); ?>" 
                   class="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-900">
          </div>
          ...
          <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
            ...
            <select id="how_hear" name="how_hear" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-900">
              ...
              <option value="social_media" <?php selected(get_post_meta($order_id, '_billing_how_heard_about_us', true), 'social_media'); ?>>Social Media</option>
              <option value="search_engine" <?php selected(get_post_meta($order_id, '_billing_how_heard_about_us', true), 'search_engine'); ?>
              <?php selected(get_post_meta($order_id, '_billing_how_heard_about_us', true), 'web_search'); ?>>Search Engine</option>
              <option value="friend" <?php selected(get_post_meta($order_id, '_billing_how_heard_about_us', true), 'friend'); ?>>Friend / Family</option>
              <option value="advertisement" <?php selected(get_post_meta($order_id, '_billing_how_heard_about_us', true), 'advertisement'); ?>>Advertisement</option>
              <option value="other" <?php selected(get_post_meta($order_id, '_billing_how_heard_about_us', true), 'other'); ?>>Other</option>
            </select>
          </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
            ...
            <select id="cleaning_type" name="cleaning_type" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-900">
              ...
              <option value="<?php echo esc_attr($product_name); ?>" <?php echo ($cleaning_type_value === $product_name) ? 'selected' : ''; ?>><?php echo esc_html($product_name); ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
      ...
      <div class="bg-gradient-to-br from-blue-50 to-purple-50 rounded-2xl p-6 border border-blue-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" id="pinAddress" name="pin_address" value="<?php echo esc_attr($billing_location ? $billing_location : ''); ?>" class="mt-3 w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-900" placeholder="Selected location address">
  ...
  <script>
    ...
    function removeImage(type) {
      ...
      .then(data => {
        if (data.success) {
          // Reload page to show the change
    ...
    function uploadImage(type) {
      ...
      parentDiv.innerHTML = '<div class="w-full h-40 bg-gray-100 rounded-xl flex items-center justify-center"><div class="text-center"><div class="spinner"></div><p class="text-sm font-medium text-gray-600 mt-2">Uploading...</p></div></div>';
    ...
    function saveOrderDetails() {
      // Show loading state
      const saveBtn = document.querySelector('[onclick="saveOrderDetails()"]');
      ...
      const flowersDelivered = document.getElementById('flowers_delivered') ? document.getElementById('flowers_delivered').value : '';
      const accessories = document.getElementById('accessories') ? document.getElementById('accessories').value : '';
      ...
      // Send request to the same page (order-details.php)
      fetch('', {
        method: 'POST',
        body: formData
      })
      ...
      .then(data => {
        if (data.success) {
          ...
          // Show error message
          showNotification('Error saving order details: ' + data.message, 'error');
        }
      })
      .catch(error => {
        console.error('Error:', error);
        showNotification('An error occurred while saving order details', 'error');
      })
      .finally(() => {
        // Reset button state - this is important to stop the preloader
    ...
    function showNotification(message, type) {
      ...
      notification.className = `fixed top-4 right-4 p-4 rounded-xl shadow-lg z-50 flex items-center gap-3 transform transition-all duration-300 translate-x-full`;
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function extractPinCode(address) {
        ...
        if (matches && matches.length > 0) {
          // Return the last match (most likely to be pin code)
      ...
      // Function to update marker position (but not save to backend)
      function updateMarkerPosition(lat, lng, address) {
        ...
        // Update form fields only (don't save to backend yet)
        ...
        // Don't save to backend here - let user click Save Changes button
        console.log('Location updated in form fields, not saved to backend yet');
      }
        ...
        .then(data => {
          ...
          data.forEach(item => {
            ...
            resultItem.addEventListener('click', function() {
              ...
              // Update marker (but don't save to backend)
              updateMarkerPosition(lat, lng, address);
